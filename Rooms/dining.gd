...
@export var wino_minigame_scene: PackedScene = preload("res://Minigames/WinoMinigame.tscn")
...
func _ready() -> void:
	...
	var wino = find_child("Wino", true, false)
	if wino:
		wino.connect("pressed", Callable(self, "_on_wino_pressed"))
	else:
		print("⚠️ Nie znaleziono obiektu 'Wino' w tej scenie!")

	if GameState.last_room == "Dining":
...
func _on_minigame_finished(item) -> void:  
	...
	if item and GlobalInventory:
		...
		print("Item added to global inventory from dining minigame!")
